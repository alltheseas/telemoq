...
mod tests {
    ...
    fn parse_telemetry_roundtrip() {
        // Build a 2-frame telemetry.bin in memory
        ...
        assert_eq!(data.len(), 120); // 2 x 60
        let frames = parse_telemetry(&data);
        assert_eq!(frames.len(), 2);
        assert!((frames[0].timestamp - 0.0).abs() < 1e-6);
        assert!((frames[1].timestamp - 0.0667).abs() < 1e-4);
        assert!((frames[0].state[0] - 0.0).abs() < 1e-6);
        assert!((frames[0].action[0] - 0.0).abs() < 1e-6);
        assert!((frames[1].state[0] - 0.7).abs() < 1e-6);
    }

    #[test]
    fn parse_telemetry_empty() {
        let frames = parse_telemetry(&[]);
        assert!(frames.is_empty());
    }

    #[test]
    fn parse_telemetry_partial_record_ignored() {
        // 59 bytes = less than one record
        let data = vec![0u8; 59];
        let frames = parse_telemetry(&data);
        assert!(frames.is_empty());
    }
